...
const messages = {
  zh: {
    ...
    'status.waiting': '等待连接',
    'status.failed': '连接失败',
    'status.opening.adb': '正在打开 ADB USB 接口…',
    'status.opening.hdc': '正在打开 HDC USB 接口…',
    'status.session.adb':
    ...
    'empty.title': '未连接设备',
    'empty.desc.harmony':
      '通过 WebUSB 连接 HarmonyOS / OpenHarmony 设备后，映射 WebView DevTools socket 并内嵌 Chrome DevTools。',
    'empty.desc.android':
      '通过 WebUSB 连接 Android 设备后，映射 WebView DevTools socket 并内嵌 Chrome DevTools。',
    'empty.hint.secure':
      '需要桌面版 Chrome / Edge，并在安全上下文（localhost / HTTPS）中使用。',
    'empty.hint.harmony':
      '若 USB 接口被占用，请先退出 DevEco Studio 或执行 hdc kill 后重试。',
    'empty.hint.android':
      '若 USB 接口被占用，请先退出本机 adb server 或手机助手；首次连接需在设备上允许 USB 调试。',
    ...
    'targets.placeholder': '点击左侧调试端口后，这里会显示其中的可调试页面。',
  ...
  en: {
    ...
    'status.opening.adb': 'Opening ADB USB interface…',
    'status.opening.hdc': 'Opening HDC USB interface…',
    'status.session.adb':
    ...
    'empty.desc.harmony':
      'Connect a HarmonyOS / OpenHarmony device over WebUSB, map its WebView DevTools socket and embed Chrome DevTools.',
    'empty.desc.android':
      'Connect an Android device over WebUSB, map its WebView DevTools socket and embed Chrome DevTools.',
    'empty.hint.secure':
      'Requires desktop Chrome / Edge in a secure context (localhost / HTTPS).',
    'empty.hint.harmony':
      'If the USB interface is busy, quit DevEco Studio or run hdc kill first.',
    'empty.hint.android':
      'If the USB interface is busy, stop the local adb server or phone assistants first; approve USB debugging on the device for the first connection.',
    'support.noWebUsb':
      'WebUSB is not supported in this browser — use desktop Chrome or Edge',
    'sidebar.ports': 'Debug ports',
    'sidebar.pages': 'Debuggable pages',
    'scan.rescan': 'Rescan',
    ...
    'targets.placeholder': 'Select a debug port on the left to list its pages.',
