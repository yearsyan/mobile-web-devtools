...
import { ByteQueue, joinBytes } from '../utils/bytes';
...
export interface ForwardedWebSocketOptions {
  timeout?: number;
  /**
   * 可选 Origin 头。Chromium 的远程调试 WebSocket 服务默认拒绝
   * 大多数页面 Origin；传 null / undefined 时按非浏览器 WebSocket
   * 客户端处理，不发送 Origin 头。
   */
  origin?: string | null;
  protocols?: string[];
  ...
  push(chunk: Uint8Array): ParsedWebSocketFrame[] {
    ...
    while (this.#buffer.byteLength >= 2) {
      ...
      if (length === 126) {
        ...
        length = new DataView(input.buffer, input.byteOffset + 2, 2).getUint16(
          0,
          false,
        );
        offset += 2;
      ...
      if (masked) {
        ...
        mask = input.slice(offset, offset + 4);
        offset += 4;
      }
...
async function expectedAccept(key: string): Promise<string> {
  const digest = await crypto.subtle.digest(
    'SHA-1',
    encoder.encode(`${key}${WEBSOCKET_GUID}`),
  );
...
export class ForwardedWebSocket {
  ...
  static async connect(
    ...
    if (
      protocols.some(
        (protocol) => !/^[!#$%&'*+\-.^_`|~0-9A-Za-z]+$/u.test(protocol),
      )
    ) {
  ...
  async #start(
    ...
    await this.#stream.write(
      encoder.encode(websocketRequest(url, this.#key, origin, protocols)),
    );
    await withTimeout(
      handshake,
      timeout,
      `等待 ${url.pathname} WebSocket Upgrade 超时`,
    );
  }
  ...
  async #drain(): Promise<void> {
    if (this.#state === 'connecting') {
      const response = tryParseHttpResponse(this.#incoming.bytes);
      if (!response) {
        return;
      }
      this.#incoming.consume(response.consumed);
      if (response.status !== 101) {
        const body = new TextDecoder()
          ...
          .slice(0, 300);
        throw new Error(
          `WebSocket Upgrade 失败：HTTP ${response.status} ${response.statusText}${
            body ? ` · ${body}` : ''
          }`.trim(),
        );
      }
    ...
    const frames = this.#parser.push(this.#incoming.bytes);
    this.#incoming.clear();
  ...
  async #handleFrame(frame: ParsedWebSocketFrame): Promise<void> {
    ...
    if (frame.opcode === 0x08) {
      ...
      const reason =
        frame.payload.byteLength > 2
          ? fatalDecoder.decode(frame.payload.subarray(2))
          : '';
      this.#state = 'closing';
      this.#closeCode = code;
    ...
    this.#callbacks.onMessage(
      opcode === 0x01 ? fatalDecoder.decode(payload) : payload,
    );
  }
  ...
  #streamClosed(error: Error | null): void {
    ...
    if (this.#state === 'connecting') {
      this.#handshakeReject?.(
        error ?? new Error('WebSocket Upgrade 前连接已关闭'),
      );
    }
    this.#state = 'closed';
    this.#finishClose();
