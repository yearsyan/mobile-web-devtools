...
function findBytes(input: Uint8Array, needle: Uint8Array, start = 0): number {
  outer: for (
    let index = start;
    index <= input.byteLength - needle.byteLength;
    index += 1
  ) {
...
function parseChunkedBody(
  ...
  while (true) {
    ...
    const sizeText =
      decoder
        .decode(input.subarray(offset, lineEnd))
        .split(';', 1)[0]
        ?.trim() ?? '';
    ...
    if (size === 0) {
      ...
      if (input[offset] === 13 && input[offset + 1] === 10) {
        return { body: joinBytes(chunks), consumed: offset + 2 };
      }
...
/** Incrementally parse one HTTP response; returns null while more data is needed. */
...
async function requestOnStream(
  ...
  const response = new Promise<ForwardHttpResponse>((resolve, reject) => {
    ...
    const parse = (eof = false) => {
      try {
        const value = tryParseHttpResponse(buffer.bytes, { eof });
        if (value) {
          succeed(value);
        } else if (eof) {
    ...
    timer = setTimeout(
      () => fail(new Error(`DevTools HTTP 请求超时：${path}`)),
      timeout,
    );
