...
/** Parse `/proc/net/unix` lines containing an abstract WebView DevTools socket. */
export function parseDevtoolsSockets(output: string): DevtoolsSocket[] {
  ...
  for (const rawLine of output.split(/\r?\n/u)) {
    ...
    const pid =
      name.match(/(?:remote|devtools)[_-](\d+)(?:\D|$)/iu)?.[1] ?? null;
    sockets.set(name, { name, pid, raw: line });
  }
  return [...sockets.values()].sort((left, right) =>
...
export function parseDevtoolsTargets(json: string): DevtoolsTarget[] {
  ...
  for (const value of values) {
    ...
    targets.push({
      ...
      devtoolsFrontendUrl:
        stringField(value, 'devtoolsFrontendUrl') || undefined,
      devtoolsFrontendUrlCompat:
        stringField(value, 'devtoolsFrontendUrlCompat') || undefined,
    });
  }
  return targets;
}
...
/** Only accept the official Chromium-hosted DevTools frontend. */
export function resolveDevtoolsFrontendUrl(
  target: DevtoolsTarget,
  version: DevtoolsVersion | null,
): string {
  const candidates = [
    target.devtoolsFrontendUrlCompat,
